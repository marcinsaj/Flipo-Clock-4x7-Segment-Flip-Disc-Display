 ...
 *                                                                                               *
 * Thermometer (Fahrenheit) & Hygrometer                                                            *
...
void DisplayTemperatureHumidity(void)
{
  ...
  float temperature = (heat_fahrenheit + 0.05);

  // ​​Preparing the temperature for display
  if(temperature < 100) temperature = temperature * 10;
...
void DisplayData(float data, uint8_t type)
{
  ...
  // If the temperature is equal to or higher than 100F, do not display a dot
  // The value is 1000 because earlier, when preparing the data for the display, 
  // we multiplied the value by 10
  if(data_value >= 1000) Flip.Display_3x1(1, 0,0,0);
